...
\textbf{Solution:} Character (Nominal). One possible way could be `+'
for positive, `N' for neutral, `-' for negative.

\hypertarget{question-2-hashtag-havoc-great-name}{%
\paragraph{Question 2: Hashtag Havoc (great
name)}\label{question-2-hashtag-havoc-great-name}}
...
\textbf{Solution:} List, where the element names are the characters of
the hashtags, and the elements themselves are integers of the number of
times the hashtag appears. ie hashtag\_havoc -\textgreater{}
list(slay=7L, iaminpain=20L, meow=9L)
...
\textbf{Solution:} Non-numeric. The timestamps might have characters
like ``-'', and might include the spelling of months and days like
``April'' and ``Tuesday'' respectively. Even if the timestamp only
includes numbers and ``.''s, there will be multiple ``.''s like
``2.23.2013'' which cannot be registered as a continuous double value
(im pretty sure)
...
\textbf{Solution:} Assumptions:

\begin{itemize}
\item
  the date spells out the month, eg ``20August2023''
\item
  the time only includes the hours and minute, ignoring the seconds,
  using the 24 hour clock and does not include timezone information, eg
  ``20.23'' for 8:23pm
\item
  the event name need not be recorded
\end{itemize}

I should use vectors for each date, such that the elements in each
vector is the timing where the event occured on that date. Then, each
vector name should be compiled in a like like child.ages=c(4,7,9))
AHHHHHHHHHH \textbf{re-write this later}

not\_elegant -\textgreater{} list(20August2023=c(5.34, 6.12,
20,47),22August2023=c(4.54, 6.22, 21.57)) or have a list for each
month??
...
\textbf{Solution:} Vectors of characters(strings).
...
\textbf{Solution:} Character (nominal strings)
...
\textbf{Solution:} Time spent on social media, in hours: double, numeric
Names of social media used: characters(strings), non-numeric Age of
participant who used social media: integer(discrete, ordinal), numeric
