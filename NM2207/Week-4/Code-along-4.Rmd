...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
glimpse(hotels) #variables stacked on top of each other
```
<br>
...
#method 2
vip_info = c("lead_time","agent","market_segment")
select(hotels, all_of(vip_info))

#to remove variables
select(hotels, -c(lead_time,agent,market_segment))
```
...
#remember for ggplot its '+' for layering, not pipe '%>%'
...
### 14. *Non-conditional* selection of rows: non-consecutive/specific *indices* using slice() (Slide #51-52)
...
### 15. Pick unique rows using distinct() (Slide #54)
...
hotels %>% mutate(little_ones = children + babies) %>%
  select(hotel, little_ones,children, babies) #double type of little_ones is automatically assigned

#hotels %>% mutate(little_ones = children + babies) %>%
#  filter(little_ones>0) %>%
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels %>% mutate(little_ones = children + babies, average_little_ones = mean(little_ones)) %>%
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels %>% count(market_segment, sort = TRUE) #decreasing order of countss
```
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels %>% summarise(mean_adr = mean(adr)) #adr is ave daily rate
```
...
hotels %>% filter(adults == 1, children >= 1 | babies >= 1) %>%
  select(adults, babies, children) # | is the or operator
```
