...
# removed `r icons::fontawesome("r-project")`bc idk how to install the icons package and if i have to look at the CRAN thing again i WILL lose it
```
...
6. <strong>Remember</strong> to set `eval=FALSE` after completing the code to generate the output
...
**Solution:** "CSV" stands for "Comma Separated Values". It is a popular format because CSV files are compatible with numerous different programs. CSV files are also very readable to humans, so CSV files are easy to create and manipulate.
<br>
<br>
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="Screenshot of R studio"}
knitr::include_graphics("C:/Users/dcsil/great-screenshot.jpeg") #can leave in same directory, then no need specify path
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** The skip argument requires an integer that refers to the number of lines to skip before reading the data in the file.
<br>
<br>
...
sales_data <- read_csv("sales_data.csv") #i think thats it lol
```
<br>
<br>
...
```{r,echo=TRUE,eval=TRUE}
length(playlist_data$DJ_Name) #assume count = number of
...
```{r,echo=TRUE,eval=TRUE}
filter_col <- c(playlist_data$DJ_Name)
playlist_data %>% filter(DJ_Name%in%filter_col) %>% select(DJ_Name, Location) %>% print(n = length(playlist_data$DJ_Name))
...
```{r,echo=TRUE,eval=TRUE}
playlist_data %>% filter(DJ_Name%in%filter_col) %>% select(Age) %>% print(n = length(playlist_data$DJ_Name))
```
<br>
<br>
...
mapping = aes(x=Age,y=Rating)) +
  geom_point() +
  labs(x = "Age", y = "Rating", caption = "Age isnt an indicator of talent babyyy")
...
ggplot(data = playlist_data, 
mapping = aes(x=Age,y=Rating)) +
  geom_point(shape=23, fill = "purple", size=3) +
  labs(x = "Age", y = "Rating", caption = "Age isnt an indicator of talent babyyy")
...
mapping = aes(x=Age,y=Rating)) +
  geom_point(shape=23, fill = "purple", size=3) +
  labs(x = "Age", y = "Rating", title = "Scatter graph of Age against Ratings", caption = "My insight is that since there are both high and low ratings for both younger and older DJs, ratings are not dependent on age")
#My insight is that since there are both high and low ratings for both younger and older DJs, ratings are not dependent on age
